...
async def encrypt(dut, pt_hex):
    """Encrypt one block.  Encryption = 221 cycles from valid_in; we wait 250."""
...
async def test_nist_fips197_c1(dut):
    """NIST FIPS 197 App.C.1 — key 000102..0f, pt 00112233..eeff → 69c4e0d8..c55a"""
    cocotb.start_soon(Clock(dut.clk, 20, unit="ns").start())
    await reset(dut)
    await load_key_bytes(dut, "000102030405060708090a0b0c0d0e0f")
    await encrypt(dut,         "00112233445566778899aabbccddeeff")
    ct = await read_ciphertext(dut)
    dut._log.info(f"Got      : {ct.hex()}")
    dut._log.info(f"Expected : 69c4e0d86a7b0430d8cdb78070b4c55a")
    assert ct.hex() == "69c4e0d86a7b0430d8cdb78070b4c55a", f"MISMATCH: {ct.hex()}"
    dut._log.info("PASS")

@cocotb.test()
async def test_nist_fips197_appb(dut):
    """NIST FIPS 197 App.B — key 2b7e..3c, pt 3243..34 → 3925..32"""
    cocotb.start_soon(Clock(dut.clk, 20, unit="ns").start())
    await reset(dut)
    await load_key_bytes(dut, "2b7e151628aed2a6abf7158809cf4f3c")
    await encrypt(dut,         "3243f6a8885a308d313198a2e0370734")
    ct = await read_ciphertext(dut)
    assert ct.hex() == "3925841d02dc09fbdc118597196a0b32", f"MISMATCH: {ct.hex()}"
    dut._log.info("PASS")

@cocotb.test()
async def test_second_encrypt_same_key(dut):
    """Two consecutive blocks — verifies key resets between encryptions."""
    cocotb.start_soon(Clock(dut.clk, 20, unit="ns").start())
    await reset(dut)
    await load_key_bytes(dut, "2b7e151628aed2a6abf7158809cf4f3c")
